...
est_met <- read_tsv(file = "~/Box Sync/Disha/Projects/carb_utilization/cinnocuum/all_cogs_modules.txt")
...
#cluster_group1$color[cluster_group1$clades == "clade_1"] <- "darkviolet"
#cluster_group1$color[cluster_group1$clades == "clade_2"] <- "green2"
#cluster_group1$color[cluster_group1$clades == "clade_3"] <- "royalblue3"
#cluster_group1$color[cluster_group1$clades == "clade_4"] <- "orange"


# Data plotting

merge_met <- inner_join(cluster_group1, est_met1, by='isolate')

merge_met_carb <- subset(merge_met, merge_met$module_category == "Carbohydrate metabolism")

ggplot(merge_met_carb, aes(x= clades, y = module_name, fill= module_completeness)) +geom_tile(color = "black") + theme_classic() +
  scale_fill_gradient2(low = "white", high = "navy", midpoint = 0.75) + coord_fixed() + theme(axis.text.x = element_text(angle = 45, hjust = 1))


merge_met_aa <- subset(merge_met, merge_met$module_category == "Amino acid metabolism")

ggplot(merge_met_aa, aes(x= clades, y = module_name, fill= module_completeness)) +geom_tile(color = "black") + theme_classic() +
  scale_fill_gradient2(low = "white", high = "navy", midpoint = 0.75) + coord_fixed() + theme(axis.text.x = element_text(angle = 45, hjust = 1))

merge_met_lipid <- subset(merge_met, merge_met$module_category == "Lipid metabolism")

ggplot(merge_met_lipid, aes(x= clades, y = module_name, fill= module_completeness)) +geom_tile(color = "black") + theme_classic() +
  scale_fill_gradient2(low = "white", high = "navy", midpoint = 0.75) + coord_fixed()
